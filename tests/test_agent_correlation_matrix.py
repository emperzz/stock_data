...
def test_inner_cache_avoids_recomputation(monkeypatch):
    ...
    def cached_kline(**kw):
        # Post days-removal the route passes explicit dates (converted from
        # the request's calendar days), so the inner-cache key is the window.
        key = (kw["stock_code"], kw["start_date"], kw["end_date"], kw["frequency"])
        if key in cache:
            return cache[key]
        fetch_calls["n"] += 1
        result = (df, "tushare")
...
def test_trailing_trim_fires_when_fetch_over_returns(monkeypatch):
    ...
    deliberately over-returns (120 rows ignoring the requested window) to prove
    the trim step itself. In production the d/w/m fetchers return ~0.7×days
    trading bars for a calendar window — FEWER than the window —
    ...
    assert r.status_code == 200
    # Fetcher got an explicit window spanning days + 1 (1 buffer for pct_change)
    from datetime import date as _date
    from datetime import timedelta

    called = mgr.get_kline_data.call_args.kwargs
    today = _date.today()
    assert called["end_date"] == today.isoformat()
    assert called["start_date"] == (today - timedelta(days=30 + 1)).isoformat()
    body = r.json()
...
def test_calendar_window_yields_real_trading_bars(monkeypatch):
    """days is a calendar-day window (spec §2.5): the route converts it to an
    explicit [today-(days+1), today] window, and the fetcher returns only
    trading (weekday) bars — FEWER than days+1 rows. trailing_window=days+1
    is therefore a no-op and common_bars reflects the real bar count, NOT days.
    The 'exactly N returns' invariant holds only for dense minute bars; for
    d/w/m it does not (calendar > trading days)."""
    from datetime import date as _date
    from datetime import timedelta

    def kline(**kw):
        # real fetcher over an explicit window → weekday bars only
        idx = pd.bdate_range(kw["start_date"], kw["end_date"])
        return (
            pd.DataFrame({"trade_date": idx, "close": np.linspace(100, 130, len(idx))}),
            "tushare",
        )
    ...
    called = mgr.get_kline_data.call_args.kwargs
    today = _date.today()
    # route resolves days+1 buffer into an explicit window
    assert called["start_date"] == (today - timedelta(days=30 + 1)).isoformat()
    assert called["end_date"] == today.isoformat()
    body = r.json()
    assert body["alignment"]["requested_days"] == 30
    expected_bars = len(pd.bdate_range(called["start_date"], called["end_date"]))
