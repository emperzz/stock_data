...
def _enable_token(fetcher: ZhituFetcher, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: "test_token" if a and a[0] == "ZHITU_TOKEN" else os.environ.get(a[0] if a else "", ""),
    )
...
class TestCapability:
    ...
    def test_methods_resolve(self):
        for m in ("get_dividend", "get_fund_flow_minute", "get_fund_flow_120d",
...
class TestGetDividend:
    ...
    def test_normalizes_payload(self, mock_get, monkeypatch):
        ...
        mock_get.return_value = _make_json_response([
            {"sdate": "2024-09-26", "give": "0", "change": "0", "send": "2.46",
             "line": "实施", "cdate": "2024-10-10", "edate": "2024-10-09", "hdate": "--"},
            {"sdate": "2024-06-06", "give": "1", "change": "2", "send": "7.19",
             "line": "实施", "cdate": "2024-06-14", "edate": "2024-06-13", "hdate": "--"},
        ])
        result = self.fetcher.get_dividend("600519")
        assert len(result) == 2
        assert result[0]["date"] == "2024-10-10"
        assert result[0]["bonus_rmb"] == 2.46
        assert result[0]["transfer_ratio"] == 0.0
        assert result[0]["bonus_ratio"] == 0.0
        assert result[0]["plan"] == "实施"
        assert result[1]["date"] == "2024-06-14"
        assert result[1]["bonus_rmb"] == 7.19
    ...
    def test_page_size_caps_results(self, mock_get, monkeypatch):
        _enable_token(self.fetcher, monkeypatch)
        mock_get.return_value = _make_json_response([
            {"cdate": f"{year}-06-23", "send": "0.50", "change": "0", "give": "0", "line": "实施"}
...
class TestGetFundFlowMinute:
    ...
    def test_normalizes_main_super_large_mid_small(self, mock_get, monkeypatch):
        ...
        mock_get.return_value = _make_json_response([
            {"t": "2025-06-23 09:35:00",
             "zmbstdcje": 1000, "zmsstdcje": 0,    # super: +1000
             "zmbddcje": 200,  "zmsddcje": 0,     # large: +200
             "zmbzdcje": 50,   "zmszdcje": 0,     # mid:   +50
             "zmbxdcje": 10,   "zmsxdcje": 0},    # small: +10
            {"t": "2025-06-23 09:40:00",
             "zmbstdcje": 0,    "zmsstdcje": 500, # super: -500
             "zmbddcje": 0,     "zmsddcje": 100,  # large: -100
             "zmbzdcje": 30,    "zmszdcje": 0,    # mid:   +30
             "zmbxdcje": 0,     "zmsxdcje": 5},   # small: -5
        ])
    ...
    def test_skips_non_dict_rows(self, mock_get, monkeypatch):
        ...
        mock_get.return_value = _make_json_response([
            {"t": "2025-06-23 09:35:00",
             "zmbstdcje": 100, "zmsstdcje": 0, "zmbddcje": 0, "zmsddcje": 0,
             "zmbzdcje": 0, "zmszdcje": 0, "zmbxdcje": 0, "zmsxdcje": 0},
...
class TestGetFundFlow120d:
    ...
    def test_normalizes_daily_records(self, mock_get, monkeypatch):
        ...
        mock_get.return_value = _make_json_response([
            {"t": "2025-06-20",
             "zmbstdcje": 1000, "zmsstdcje": 200,
             "zmbddcje": 500,  "zmsddcje": 100,
             "zmbzdcje": 50,   "zmszdcje": 10,
             "zmbxdcje": 5,    "zmsxdcje": 1},
        ])
