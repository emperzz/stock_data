...
**item 字段**（实测）：`summary, code, titleColor, realSort, showTime, title, share, pinglun_Num, stockList, image`。其中 `code` 是东财内部文章 ID（19 位数字字符串如 `202606223778033979`），不是股票代码 —— 用于拼出详情页 URL。
