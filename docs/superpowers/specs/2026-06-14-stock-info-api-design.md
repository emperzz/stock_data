...
### 2.2 Myquant —— `gm.api.stk_get_instrumentinfos`

- 鉴权：`MYQUANT_TOKEN` 环境变量
- 通过 `gm` SDK 调用，免 HTTP
- `stk_get_instrumentinfos` **当前未实现**——是本次新加的端点

**`stk_get_instrumentinfos(symbols='SHSE.600519', df=True, fields=...)` 可用字段**：
`name`, `exchange`, `listed_date` (epoch ms), `delisted_date` (epoch ms), `total_share` (股), `float_share` (股), `industry`

**关于 `get_symbols`**：早期误以为是此 API——经核实 `get_symbols(sec_type1=1010, df=True)` 是拉**全市场**股票列表用的，不支持按 `symbols=` 过滤单只股票。故本设计改用 `stk_get_instrumentinfos` 单端点一次性拉所有需要的字段（无需分两步）。
...
def get_stock_info(self, stock_code: str) -> dict | None:
    ...
    try:
        ...
        # 一次性拉所有需要的字段（name / listed_date / total_share / industry ...）
        df = gm.api.stk_get_instrumentinfos(
            symbols=symbol_full, df=True,
            fields="name,listed_date,delisted_date,total_share,float_share,industry",
        )
...
> **注**：Myquant 的 `stk_get_instrumentinfos` 单次调用同时返回 `name/listed_date/total_share/industry` 等所有需要的字段，不需要分两步。`industry` 字段拉取失败由外层 `try/except` 兜住，不会阻塞整体返回（`row.get("industry", "")` 即空字符串）。
...
- `test_normalizes_full_payload` — `mock.patch("gm.api.stk_get_instrumentinfos")` 返手搓 DataFrame，断言毫秒时间戳 → `YYYY-MM-DD`、股 → 万股 ÷ 10000、industry 填入、concepts 留空 list
- `test_returns_none_on_empty_df` — `stk_get_instrumentinfos` 返空 df → `None`
- `test_industry_missing_does_not_block_response` — DataFrame 中 `industry` 列为 NaN 时，主流程仍返回（industry 落空串），不降级为 `None`
