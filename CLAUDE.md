...
| `POST /agent/stocks/board-overlap` | Pairwise board-set intersection + Jaccard across 2-10 stocks. | per-code `stock_board_cache.get_stock_memberships(sources=['ths'])`; **cold ths index → live THS reverse lookup** via `_helpers.stock_boards.fetch_stock_boards_quote_enrichment` (shared 60s cache). `sets[].source` is three-state `persistence` / `ths` / `unavailable`; on `unavailable` the code lands in `errors[]` and pairs touching it carry `jaccard: null` + `intersection_count: null` (**never 0.0** — see the board-overlap contract below) |
...
| `GET /agent/market-stats` | 全市场涨幅统计（个股 + 板块；均值/中位/最高/最低/上涨下跌家数 + 桶形数据）。`boards` 块额外含 `top_gainers` / `top_losers`（各 ≤3 条 `BoardMoverEntry`，**平铺**——2026-09-09 修订，去掉了嵌套 `MinimalQuote` 与 subtype/source/platecode；quote 字段平铺在 code/name/type 之后，amount 亿元 / volume 万手 / net_inflow 亿元），按 change_pct DESC/ASC 排序，tie-break = code ASC，None change_pct 排除（post-2026-09-09）。 | `manager.get_realtime_quotes('csi')` + `stock_board_cache.get_board_list(board_type=None, source='ths', include_quote=True, manager=manager)`; 60s TTLCache via `get_quote_cache`; per-block 错误隔离。冷路径 `asyncio.gather` 扇出 3 个块(sync helper 走 `asyncio.to_thread`);两条编排路径(sync wrapper 供 market-recap / async 路由)统一收口到 `_assemble_market_stats_response`。 |
...
| `GET /agent/lead-stocks` | 涨停龙头股服务端排名（连板数×**limit band 涨幅**→最后涨停时间→封单金额）。`change_pct ∈ [9, 11]` 视为 10cm、`[19, 22]` 视为 20cm，中间 `(11, 19)` 与两端外部排除；`score = lb × limit_pct`（10 或 20），让 20cm 自然 2× 高于 10cm 同一连板数。默认 top_n=3，范围 [1, 20]；可选 `board_code` 交集。**不**返回 computed technical indicators (`features` 字段在 2026-09-06 spec 修订中移除，response 仅含 quote / info / boards)。 | `manager.get_zt_pool("zt", date)` + 可选 `stock_board_cache.get_board_stocks(source="ths")` + `manager.get_zt_reasons(date)`（降级）+ `build_stock_profile(..., include_features=False)` per top_n code |
...
- **"Unknown" must never be emitted as a number (board-overlap).** The ths reverse index ships **empty** (no THS membership seed — see `docs/board-source-semantics.md`), so the persistence read is cold on a fresh DB for *every* stock. `POST /agent/stocks/board-overlap` therefore falls back to the live THS reverse lookup and labels each set with `source`: `persistence` (warm index) / `ths` (live fallback answered — an empty `boards` here IS a fact) / `unavailable` (both failed → membership UNKNOWN). On `unavailable` the code goes into `errors[]` with `error="ths_reverse_lookup_unavailable"`, and every pair touching it MUST carry `jaccard: null` + `intersection_count: null`. **Do not collapse `unavailable` into `0.0`/`0`** — that converts "we could not look this up" into the false fact "these stocks share no boards". Measured 2026-09-14: the endpoint reported `0.0` where the truth was 20 common boards (jaccard 0.196). `?format=md` renders the nulls as `未知` and the empty-but-known case as `（无所属板块）`. Pinned by `tests/test_agent_endpoints.py::TestStocksBoardOverlap` (`test_cold_index_and_failed_live_lookup_reports_unknown_not_zero`) + `TestFormatMdDataCompleteness::test_stocks_board_overlap_unknown_set_renders_marker`.
...
- **Don't** reorder decorators on a route. The required order is `@router.get → @endpoint_meta → @map_errors → @cache_endpoint → def|async def` (`@endpoint_meta` OUTER relative to `@map_errors`/`@cache_endpoint`, INNER relative to `@router.get`; see `api/routes/news.py:135`). This works only because `endpoint_meta.deco` returns the original `func`, so `route.endpoint` IS the `REGISTRY` key. Break either and the route silently vanishes from the explorer manifest — no error. (`explorer/manifest.py::_lookup_registry` walks `__wrapped__` defensively, but that path is unused today.)
- **Don't** wrap an `async def` route handler in a *sync* decorator, and don't "simplify" `map_errors` back to a single sync wrapper. `map_errors` branches on `inspect.iscoroutinefunction` and returns an `async def` wrapper for coroutine handlers precisely so the `await` happens **inside** its try/except. A sync wrapper still passes the happy path — FastAPI decides whether to await by unwrapping `__wrapped__`, so the coroutine does run — but it runs at FastAPI's `await` site, outside the try/except, so `DataFetchError→503` / `ValueError→400` / generic `→500 {"error":"internal_error"}` all silently degrade to a plain-text 500. That hole sat undetected on `/agent/market-recap` + `/agent/correlation/matrix` from 2026-09-03 and hit `/agent/market-stats` the moment it went async (2026-09-10). Pinned by `tests/test_map_errors_async_handlers.py` — its structural test fails for any route whose decorator stack flips sync↔coroutine.
