...
| `GET /agent/lead-stocks` | 涨停龙头股服务端排名（连板数×涨幅→最后涨停时间→封单金额；`change_pct ∈ [9, 22]` 排除 30cm/ST）。默认 top_n=3，范围 [1, 20]；可选 `board_code` 交集。**不**返回 computed technical indicators (`features` 字段在 2026-09-06 spec 修订中移除，response 仅含 quote / info / boards)。 | `manager.get_zt_pool("zt", date)` + 可选 `stock_board_cache.get_board_stocks(source="ths")` + `manager.get_zt_reasons(date)`（降级）+ `build_stock_profile(..., include_features=False)` per top_n code |
