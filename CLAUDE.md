...
| `GET /agent/market-stats` | 全市场涨幅统计（个股 + 板块；均值/中位/最高/最低/上涨下跌家数 + 桶形数据）。`boards` 块额外含 `top_gainers` / `top_losers`（各 ≤3 条 `BoardMoverEntry`，含 `MinimalQuote`），按 change_pct DESC/ASC 排序，tie-break = code ASC，None change_pct 排除（post-2026-09-09）。 | `manager.get_realtime_quotes('csi')` + `stock_board_cache.get_board_list(board_type=None, source='ths', include_quote=True, manager=manager)`; 60s TTLCache via `get_quote_cache`; per-block 错误隔离。 |
