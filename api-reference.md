...
| `results[].boards` | object \| null | `{source, data}` board-membership envelope. `data[]` carries the same entry contract as `/stocks/{stock_code}/boards` (5 legacy + 7 THS `change_pct` / `up_count` / `down_count` / `limit_up_count` / `limit_down_count` / `explain` / `relevance`). The 7 enrichment fields may be absent on individual entries — use `"key" in entry` (not `entry.get("...") is None`) to test presence. |
