...
| 类别 | 数据形态 | 推荐取数路径 | 时段差异 |
|---|---|---|---|
| **指数全景** | 上证 / 深证成指 / 创业板指 —— 行情 + 计算指标（trend/pivots/volume），单 frequency | `§9.1` `agent/indices/batch-profile`（默认 3 指数；单 `frequency`，要长短期各调一次） | `pre`：昨日收盘 + 隔夜外盘；`intra`：当日分时 + 5m features；`post`：当日收盘 + features（要长短期趋势则分 `frequency` 调） |
