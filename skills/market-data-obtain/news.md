...
> 主文件 [§10 新闻 / 消息](../market-data-obtain.md) 的端点明细；字段、单位、调用约束、示例见本文。
...
| `from`（query, 别名 `from_`） | string | ❌ | — | 起始日期 `YYYY-MM-DD`；与 `to` 同给时必须 `≤ to`，否则 400 |
| `to`（query） | string | ❌ | — | 结束日期 `YYYY-MM-DD` |
| `limit`（query） | int | ❌ | `20` | 返回条数，1-100 |
...
| `snippet` | string | — | 摘要；可能为空字符串 |
| `media_name` | string | — | 来源媒体名（如「证券时报网」），可能为空 |
...
# 限定日期范围
curl 'http://localhost:8888/api/v1/news/search?q=600519&from=2026-05-20&to=2026-05-25&limit=20'
```
...
| `byte_size` | int | 字节 | body 长度 |
| `content_status` | string | — | `ok` / `empty` / `unsupported` / `javascript_required` / `blocked` / `fetch_error` |
| `reason` | string | — | 失败原因（仅非 `ok` 时非空） |
| `canonical_url` | string | — | URL 跳转后的最终地址（抓取诊断） |
| `http_status` | int | — | HTTP 状态码（抓取诊断） |
...
个股相关新闻列表。结构比 `/news/search` 简（无 `snippet` 字段；无全文）。
...
- `body_text` 是完整正文（纯文本）——agent 拿全文做总结的主字段
...
| `subject_id` | int | — | 固定 `1151` |
| `date` | string | — | 入参回显 `YYYY-MM-DD` |
| `article` | object | — | 文章对象（见下） |
| `article.article_id` | int | — | 文章 ID |
| `article.title` | string | — | 标题 |
| `article.brief` | string | — | 简介 |
| `article.author` | string | — | 作者 |
| `article.date` | string | — | 发布日期 `YYYY-MM-DD` |
| `article.ctime` | int | epoch 秒 | 发布时间 |
| `article.read_num` | int | — | 阅读数 |
| `article.comments_num` | int | — | 评论数 |
| `article.share_num` | int | — | 分享数 |
...
财联社焦点复盘。**与早报相同的 28 天窗口限制与 date 必填约束**。

字段结构与 `/news/morning-briefing` 完全一致，**仅 `subject` 与 `subject_id` 不同**。
