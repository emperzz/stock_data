...
> 本文件是 `market-data-obtain` 主文件 [§11 其他](../market-data-obtain.md) 的端点明细。  
...
| `sources[].last_success_time` | number | epoch 秒 | 最近一次成功时间；缺数据为 `null` |
| `sources[].last_failure_time` | number | epoch 秒 | 最近一次失败时间；缺数据为 `null` |
| `sources[].failure_count` | number | — | 累计失败次数 |
...
| `indicators[].default_lookback` | number | — | 预热所需最少 K 线根数（路由层自动 `max(days, lookback)` 拉更多再截断） |
...
```bash
curl 'http://localhost:8888/api/v1/stocks/600519/kline?frequency=d&days=60&indicators=ma,macd,kdj'
```
