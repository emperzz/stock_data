...
**`boards` 块额外含 top-3 movers**（2026-09-09 新增；同日修订为**平铺**）：`top_gainers` + `top_losers` 各 ≤3 条 `BoardMoverEntry`——quote 字段**平铺**在 `code`/`name`/`type` 之后（**无嵌套 `MinimalQuote`**；单位走板块清单语义：amount 亿元 / volume 万手 / net_inflow 亿元，非 quote 端点 元），按 change_pct DESC/ASC 排序、tie-break = code ASC、None change_pct 排除。复用了板块块已经取到的 dict，**零额外上游调用**。字段表见下 + `api-reference.md` §`BoardMoverEntry field inventory`。
...
| `boards.top_gainers` | array | — | **新增（2026-09-09；同日平铺化）**涨幅前三板块，每条 `BoardMoverEntry` **平铺** = `code` / `name` / `type` / `change_pct`(%) / `amount`(亿元) / `volume`(万手) / `up_count` / `down_count` / `net_inflow`(亿元)；按 change_pct DESC 排序、tie-break = code ASC；最多 3 条，N<3 时按实际数；`include_boards=false` 或上游失败时该字段不存在；空上游 → `[]`。概念 movers 通常只有 `change_pct` + `net_inflow`，其余为 null |
