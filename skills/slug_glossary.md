...
> **参考资料，非 skill**。配套 `market-files.md 附录 A` 使用——agent 创建主线 / 龙头 / 板块 / 事件 / 假设的稳定标识符（`标识` 字段）时，优先查本表。
>
> **必读内容**：slug 硬规则 + 创建工作流见 [market-files.md 附录 A](./market-files.md#附录-aslug-命名约束强制)。
>
...
> 工作流（5 步：识别范围 → 查本词汇表 → 全局查重 → 写命名说明 → changelog 记录）见 [market-files.md 附录 A](./market-files.md#附录-aslug-命名约束强制) 的"slug 创建工作流"。本表只提供词汇映射，不重复工作流。
...
> 本节是**历史快照，只作"正确的 slug 长什么样"的命名范例**，**不随主线淘汰 / 状态变化同步更新**——主线的增删改只发生在 `market_tracking.md`，不回流到本表。因此 agent **不需要维护**本区的时效性。
...
## 8. 已废弃 slug（只读审计，永不主动清理）

> 本区记录已合并 / 已废弃的 slug，**只读、永不主动删除**——变更轨迹由 git 历史追溯，agent 不做清理动作。改名 / 合并仍走 `MERGED → 新slug`，追加到这里即可。
...
- **补充新 L1/L2/L3**：实战中出现的新行业缩写 / 核心词映射 / 驱动因素，及时加入本表（**这是唯一需要主动做的维护动作**）
- **changelog 同步**：词汇表变更时，agent 在 tracking changelog 追加 `[glossary_update]` 记录
- **不做的维护**：不做年度 review、不清理 §8 已废弃区（只读审计，靠 git 历史追溯）、不同步 §7 样本区（历史快照）
